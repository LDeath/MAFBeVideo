 ...
 */-(instancetype)initWithFrame:(CGRect)frame ButtonType:(ButtonType)type
{
    ...
    self.titleLabel.textColor=[UIColor colorWithRed:159/255.0 green:159/255.0 blue:167/255.0 alpha:1];
...
-(void)addTarget:(id)target action:(SEL)action forResponseState:(ButtonClickType)state
{
    handel=target;
    switch (state) {
        case ButtonClickType_TouchDownPress:
            touchDownAction=action;
            break;
        case ButtonClickType_TouchUpInside:
            touchUpSideAction=action;
            break;
        default:
            break;
    }
...
- (void)longPressGesture:(UILongPressGestureRecognizer*)longGesture
{
    ...
    if ((longGesture.state==UIGestureRecognizerStateEnded||longGesture.state==UIGestureRecognizerStateCancelled)) {
        if (handel&&[handel respondsToSelector:touchUpSideAction ]) {
            if(tag < self.pathArray.count)
                self.selectButtonPosition=[self GetPositonWithTag:tag];
            [handel performSelector:touchUpSideAction withObject:self afterDelay:0];
        }
    }else if((longGesture.state==UIGestureRecognizerStateBegan))
    {
        if (handel&&[handel respondsToSelector:touchDownAction ]) {
            if(tag < self.pathArray.count)
                self.selectButtonPosition=[self GetPositonWithTag:tag];
            [handel performSelector:touchDownAction withObject:self afterDelay:0];
        }
    }
    //
    //
